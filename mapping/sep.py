...
def separator(angle_, refit = True, npoints = 1001, a = 1):
    ...
    circle = pl.circle_point(np.linalg.norm((c_1-c_0)/2),  - np.pi + np.linspace(0,2*np.pi,20000), a = a) + cusp_center[:,None]
    cuspolygon = sh.Polygon([p for p in circle.T])  ## this is dumb, I could simply check if ||p|| < r but I will replace this by a different polygon in the long run.
    ...
    if refit:  ## fit the spline back to a npoints pointcloud with equally spaced points
        bottom, top, left, right = [ pl.ndspline(rep.discrete_length_param(pc),pc,k = 1)(np.linspace(0,1,npoints)) for pc in [bottom, top, left, right]]
