...
def constrained_arc_length(leader, follower, fac = 16, smooth = False, return_func = False):
    ...
    if not smooth:
        ...
        ind1, ind2 = [[i[j] for i in indices] for j in [0,1]]
        values = verts1[ind1]
        end = follower.shape[1]
        spline = sp.interpolate.PchipInterpolator(ind2,values)
        if len(spline.derivative().roots()) > 0:
            log.warning('Warning, the reparameterization spline contains singularities')
        if return_func:
            return verts1, spline
        verts2 = spline(np.arange(0,end))
    return verts1, np.array(verts2)

def constrained_arc_length_function(leader, follower, fac = 16):
    indices = match_minimum(leader, follower, fac = fac)
    assert all([m in indices for m in [[0,0], [leader.shape[1] - 1, follower.shape[1] -1]]])
    verts1, verts2 = [discrete_length_param(pc) for pc in [leader,follower]]
    indices = np.array(indices, dtype = int)
    ind1, ind2 = [list(indices[:,i]) for i in range(2)]
    values = verts2[ind2]
    vertices = verts1[ind1]  ## the values of the cord-length parameterization at the matching indices is mapped onto values
    rep_spline = scipy.interpolate.PchipInterpolator(vertices, values)
    if len(rep_spline.derivative().roots()) > 0:  ## the reparam-spline contains singularities - add vertices
        log.warning('Warning, the reparameterization spline contains singularities, artifically adding values')
        values = verts2
        vertices = constrained_arc_length(leader,follower, fac = fac, smooth = False)[1]
    return lambda g: ut.interpolated_univariate_spline(verts1, leader, g), \
           lambda g: ut.interpolated_univariate_spline(verts2, follower,\
                                                       ut.PchipInterpolator(vertices, values, g)[0])
