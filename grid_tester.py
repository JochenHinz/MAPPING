...
def flatten_weights_to_gismo(go):
    ...
    l = len(s) // 2
    n,m = [len(kn.knots) + p - 1 for kn in knots]
...
def export_to_gismo(go):
    ...
    knots_xi, knots_eta = [(str(kn.extend_knots(go.degree))[1:-1]).replace(',','') for kn in go.knots]
    string = '<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n' \
            ...
            '<KnotVector degree=\"' + str(p) + '\"> ' + knots_eta +' </KnotVector> \n' \
            '     </Basis> \n' \
            '    </Basis> \n' \
            '    <coefs geoDim=\"2\"> \n'\
